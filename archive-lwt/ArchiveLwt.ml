...
module Read = struct
  ...
  let create input =
    ...
    in
    Lwt_preemptive.detach StdRead.create input'
  ...
  let content t fn = Lwt_preemptive.detach (StdRead.content t) fn

  let with_file t fn finit fread fclose =
    finit (entry t fn) >>= fun data ->
      ...
      Lwt_unix.make_notification (fun () ->
          Lwt.ignore_result
            (let str = Queue.pop q in
             fread !rdata str (String.length str) >|= fun data -> rdata := data))
    in
    let fread' () buf len =
