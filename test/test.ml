...
let ls_alR arch is_dir readdir = 
  let rec ls_alR' acc dn =
    Array.fold_left
      (fun acc bn ->
         let fn = 
           FilePath.concat dn bn
         in
           if is_dir arch fn then
             ls_alR' acc fn
           else
             fn :: acc)
      acc
...
let read_tarball arch dump_fn = 
  let lst = 
    ls_alR arch 
      Archive.Read.is_directory
      Archive.Read.readdir 
  in
  let dump =
    Archive.Read.content arch dump_fn
  in
    lst, dump

let read_tarball_lwt input dump_fn =
  ArchiveLwt.Read.create input 
  >>= fun arch ->
  begin
    let lst = 
      ls_alR arch 
        ArchiveLwt.Read.is_directory
        ArchiveLwt.Read.readdir 
    in
      ArchiveLwt.Read.content arch dump_fn
      >|= fun ctnt ->
      (lst, ctnt)
  end
